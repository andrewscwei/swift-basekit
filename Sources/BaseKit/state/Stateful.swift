...
///
/// When transforming this property into a state, you can optionally assign more a `StateType` to it
/// by specifying the first parameter of this property wrapper. When the value of the state is
/// changed, its key path (relative to the property owner) is marked as dirty along with all its
/// associated state types.
///
/// To conditionally prevent a new value to be assigned, you can specify the `willSet` parameter, a
/// block with the old value and new value as arguments, which must be satisfied whenever a new
/// value is assigned to the state. If the block returns `true`, the new value will be assigned. If
/// `false`, the new value will not be assigned. Note that in either case, Swift's built-in `didSet`
/// block will still be triggered for the wrapped property.
///
...
/// value was changed. You can omit this parameter and achieve the same behavior with the built-in
