...
/// A helper object that checks if any of the states or state types managed by a `StateMachine` is
/// dirty.
public struct StateValidator {
  ...
  /// Checks if the provided states are dirty in the current `StateMachine` update cycle by looking
  /// up their respective key paths.
  ///
  ...
  ///
  /// - Parameter types: The state types. This can be a signle `StateType` or an option set
  ///                    `StateType`, either works.
  ///
