...
/// `WeakReference`, there is little meaning to that because the "weakly referenced" object gets passed around as a
/// value regardless.
public struct WeakReference<T> {
  ...
  ///
  /// - Parameter object: The target object to store as a weak reference. Note that while value types can be wrapped in
  ///                     a `WeakReference`, there is little meaning to that because the "weakly referenced" object gets
  ...
  /// object will still exist in memory when invoking this method (this does not apply if the wrapped object is a value
  /// type).
  ///
